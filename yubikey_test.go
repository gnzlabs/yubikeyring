...
import (
	...
	yubikeyring_ "github.com/gnzlabs/yubikeyring/errors"
)
...
func TestGetPublicKey(t *testing.T) {
	if yk, err := openYubikey(t); err == nil {
		defer yk.Close()
		if err := generatePrivateKeys(yk); err == nil {
			if _, err := yk.GetPublicKey(keyring.ManagementKeySlot); err != nil && err == errors.ErrExportNotAllowed {
...
func TestSetCertificate(t *testing.T) {
	t.Errorf("%s", yubikeyring_.ErrNotYetImplemented)
}
